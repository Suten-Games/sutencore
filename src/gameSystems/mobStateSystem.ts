...
import { chase } from "../gameFunctions/chase";
    ...
    update(dt: number) {
        if (this.life) {
            ...
            for (let mob of mobs.entities) {
                ...
                if (!state.mobdead) {
                    log(`mobStateSystem.ts:85 - The mobdead state is false`)
                    ...
                    log(`mobStateSystem:93 - Adding FollowSystem to the mob`)
                    ...
                    if (state.mobdead) {
                        ...
                        if (state.trackplayer) {
                            ...
                            if (following) {
                                log("mobStateSystem.ts:153 - patrolling, removing chaser");
                            ...
                            if (
                                ...
                                log(`mobStateSystem.ts:164 - MOB ${mob.getComponent(NpcId).id} should be patrolling`);
                                idle.playing = false;
                                idle_.playing = false;
                                fight.playing = false;
                                fight_.playing = false;
