...
export class WarriorsTome {
    ...
    private _slot7: any;
    private _slot8: any;
    private _mywarriorstomecontents: Array<Item>;
    private obj = Singleton.getInstance();


    constructor(canvas: any, image: any) {
        //log('in the warriorstome constructor')
        let obj = Singleton.getInstance()
        this._canvas = canvas;
        this.obj.canvas = canvas;
        this._image = image;
        //this._mywarriorstomecontents = obj.playerwarriorstome;
        this._mywarriorstomecontents = obj.fetchwarriorstome();
    ...
    public bootLoadWarriorsTome(data: any[]) {
        //log(`debug: 13 Inside bootwarriorstome `, data)
        ...
        data.forEach(element => {
            if (element.slot) {
                ...
                //item.updateLoc(slot)
                //log('pushing the new spell to the warriorstomecontents array ', this._mywarriorstomecontents)
                this._mywarriorstomecontents.push(item)
    ...
    public scribeSpell(spellname: string) {
        ...
        this._mywarriorstomecontents.push(spell)
    ...
    public show() {
        log(`warriorstome.ts:152 - In showmethod()`)
        this._bp.visible = true;
        this._closebutton.visible = true;
        log(`warriorstomets:155 - Calling getContents method`)
        this.getcontents()
    }

    public hide() {
        log(`warriorstome.ts:212 - Clicked hide in the warriorstome`)
        this._bp.visible = false;
        this._buybutton.visible = false;
        this._closebutton.visible = false;
        this._buytext.visible = false;
        for (let spell of this._mywarriorstomecontents) {
            spell.hide()
        }
    }
}