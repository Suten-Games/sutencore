...
if [ -f "$SUTEN_TS" ]; then
  ...
  # Replace the placeholders or lines in the new suten.ts file before copying
  sed -i "s/^export var sutenBase = .*$/export var sutenBase = '$sutenBase';/" suten.ts
  sed -i "s/^export var duatX = .*$/export var duatX = $duatX;/" suten.ts
  sed -i "s/^export var duatZ = .*$/export var duatZ = $duatZ;/" suten.ts
fi
